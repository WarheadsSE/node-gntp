...
Message.prototype.parse = function (mimeString){
    if (typeof mimeString !== 'string'){
        return;
    }
    var len = mimeString.length;
    var prev = 0, curr = 0, lines = 0;
    var sections = 0, blanks = 0;
    var header =  new Header(); // i need to fix this object...
    
    while(curr < len && curr > -1 && blanks < 2){
        lines++;
        curr = mimeString.indexOf('\r\n',prev);
        if( curr > -1 ){
            var line = mimeString.substring(prev,curr);
            // GNTP protocol header
            if( lines == 1 ){
                var gap = mimeString.indexOf(' ');
                var protocol = mimeString.substring(0,gap).split('/');
                if( protocol[0] != Protocol.name || protocol[1] != Protocol.version ){
                    console.log('bad protocol/version');
                    return;
                }
                this.type = mimeString.substring(gap+1,mimeString.indexOf(' ',gap+1));
...
Message.prototype.mimeString = function () {
    ...
    console.log('buffer:'+bBuff.length);
...
Client.prototype.sendMessage = function (request,reponseCallback){
    ...
    sock.on('connect',function () {
        ...
        var mime = request.mimeString();
        console.log('protocol:'+protocol);
        console.log('mesg:'+mime.toHex());
        fs.writeFileSync('encrypt.log',mime);
        
        sock.write(protocol,'utf8');
        sock.write(mime);
        sock.write(EndLine+EndLine,'utf8');
    });
...
Crypto.prototype.setKeyString = function (keyHash,saltHex){
    this.salt = new Buffer(saltHex.length/2).writeHex(saltHex);
...
Crypto.prototype.encryptBuffer = function (mime) {
    ...
    var ret = crypto.update(mime,'utf8','hex');
