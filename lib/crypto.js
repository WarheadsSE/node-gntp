...
Crypto.prototype.isValidCryptoAlgorithm = function (algorithm){
    switch(algorithm){
        case 'AES':
        case 'DES':
        case '3DES':
...
Crypto.prototype.isValidHashAlgorithm = function (algorithm){
...
Crypto.prototype.encryptBuffer = function (mime) {
    ...
    if( this.encryptAlg.toUpperCase() === '3DES') { alg = 'des3'; }
    var cipher = new crypt.createCipheriv(alg,keyB.toString('binary').substr(0,keyLen),this.iv.toString('binary').substr(0,ivLen));
    
    var ret = cipher.update(mime,null,'hex');
...
Crypto.prototype.decryptBuffer = function (mime) {
    ...
    if( this.encryptAlg.toUpperCase() === 'DES') { alg = 'des'; keyLen = 8; }
    if( this.encryptAlg.toUpperCase() === '3DES') { alg = 'des3'; }
    var cipher = new crypt.createDecipheriv(alg,keyB.toString('binary').substr(0,keyLen),this.iv.toString('binary').substr(0,ivLen));
    
    var ret = cipher.update(mime,null,'binary');
